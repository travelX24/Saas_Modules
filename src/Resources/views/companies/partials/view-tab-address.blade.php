<div class="space-y-3">
  ...
  <div
      class="space-y-2"
      x-data="viewLocationModal({{ $company->id }}, {{ (float)$company->lat }}, {{ (float)$company->lng }})"
      x-on:company-updated.window="onCompanyUpdated($event)"
  >
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-3">
          <div>
              <label class="block text-xs font-semibold text-gray-600 mb-1">{{ tr('Lat') }}</label>
              <div class="text-sm text-gray-900 bg-gray-50 rounded-xl px-3 py-2 border border-gray-200 min-h-[38px] flex items-center">
                  <span x-text="lat"></span>
              </div>
          </div>

          <div>
              <label class="block text-xs font-semibold text-gray-600 mb-1">{{ tr('Lng') }}</label>
              <div class="text-sm text-gray-900 bg-gray-50 rounded-xl px-3 py-2 border border-gray-200 min-h-[38px] flex items-center">
                  <span x-text="lng"></span>
              </div>
          </div>
      </div>
          ...
          >
              ...
              <div class="flex-1 relative min-h-[calc(100vh-200px)] sm:min-h-[400px]">
                  <div
                      ...
                      @mousedown.stop @mouseup.stop @click.stop @wheel.stop @touchstart.stop
                  ></div>
              </div>
          </div>
      </div>
